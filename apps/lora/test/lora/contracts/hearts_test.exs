  ...
  describe "calculate_scores/4" do
    ...
    defp create_taken_with_hearts(hearts_distribution) do
      ...
      |> Map.new(fn {seat, heart_count} -> 
        heart_cards = if heart_count > 0 do
          # Create tricks with appropriate number of hearts
          hearts = Enum.map(1..heart_count, fn i ->
            rank = cond do
              i == 1 -> :ace
              i == 2 -> :king
              i == 3 -> :queen
              i == 4 -> :jack
              true -> 14 - i # 10, 9, 8, 7 for i=5,6,7,8
            end
            {:hearts, rank}
          end)
          ...
          tricks = Enum.map(hearts_per_trick, fn [heart] ->
    ...
    test "handles fewer than 8 hearts in play" do
      ...
      taken = create_taken_with_hearts(%{
        1 => 3,  # Player 1 has 3 hearts
        2 => 2,  # Player 2 has 2 hearts
        3 => 2,  # Player 3 has 2 hearts
        4 => 0   # Player 4 has no hearts
      })
    ...
    test "correctly processes nested trick structure" do
      ...
      assert scores == %{
        1 => 1,  # 1 heart (ace)
        2 => 2,  # 2 hearts (king, queen)
        3 => 0,  # 0 hearts (no tricks taken)
        4 => 1   # 1 heart (10)
      }
    end
  ...
  describe "integration tests" do
    test "hearts scores integrate with game progression" do
      ...
      game = %Game{
        ...
        contract_index: @hearts_contract_index,
        dealer_seat: 1,
        scores: %{1 => 10, 2 => 5, 3 => 8, 4 => 12},  # Existing scores from previous deals
        taken: %{
    ...
    test "hearts contract respects trick winning rules" do
      ...
      game = %Game{
        ...
        contract_index: @hearts_contract_index,
        trick: [
          {1, {:clubs, :jack}},    # Player 1 leads clubs
          {2, {:clubs, :ace}},     # Player 2 plays higher club
          {3, {:hearts, :king}}    # Player 3 plays off-suit (heart)
        ],
        current_player: 4,
        hands: %{
          1 => [{:diamonds, 8}],  # Players still have cards (deal not over)
          2 => [{:diamonds, 9}],
          3 => [{:diamonds, 10}],
          4 => [{:clubs, 7}]
        },
      ...
      {:ok, updated_game} = Hearts.play_card(
        game, 
        4, 
        {:clubs, 7}, 
        game.hands
